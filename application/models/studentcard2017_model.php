<?php
	...
	*/
	class studentcard2017_model extends CI_Model
	{
		...
		public function get_all_students_by_venue_id($venue_id){
			return $this->db->where('venue_id', $venue_id)->limit(100)->get('ikmc_students')->result_array();
		}
		public function get_all_venues(){
			return $this->db->get('ikmc_venues')->result_array();
		}
		...
		public function getSchoolname($id){
			return $this->db->select('name')->where('id',$id)->get('ikmc_schools')->first_row('array');
		}
		public function getVenueName($id){
			return $this->db->select('name')->where('id',$id)->get('ikmc_venues')->first_row('array');
		}
		public function getImso2017Students(){
			return $this->db->get('students_round1')->result_array();
		}
		public function getImsoStudent($sbd){
			return $this->db->where('sbd',$sbd)->get('students_round1')->first_row('array');
		}
