		...
		<section class="card col-md-8 col-xs-12 col-sm-8 col-md-offset-2 col-sm-offset-2">
			...
			<div class="card-content">
				<div class="location">
					...
					<p class="id">Percentile: <span class="highlight-txt"><?php echo round(str_replace(",", ".", $student['percentile']),2) ?>%</span></p>
					<p style="font-size: 14px">(Điểm của thí sinh cao hơn <?php echo round(str_replace(",", ".", $student['percentile']),2) ?>% số thí sinh dự thi cùng khối)</p>
					 <?php if($student['prize']==""){ ?> <span class="highlight-txt">Chúc thí sinh đạt kết quả cao hơn ở các kỳ thi sau!</span>	<?php	}else{ ?>
					<p class="prize">Giải thưởng:
					
				<span class="highlight-txt"><?php if(strpos($student['prize'],'Top')!==false){
					echo $student['prize'].' - '.'Khối '.$student['grade'];
					}else{
						echo $student['prize'];
						}?></span></p><?php } ?>
